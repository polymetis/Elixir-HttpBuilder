if Code.ensure_loaded?(MachineGun) do
    defmodule HttpBuilder.Adapters.MachineGun do
        alias HttpBuilder.HttpRequest
        
        @moduledoc """
        An adapter for using MachineGun. Expects a JSON parser to be
        included in the HTTPBuilder configuration for encoding requests. 
        
            config :http_builder, HttpBuilder.Adapters.MachineGun
                json_parser: HttpBuilder.Adapters.JSONParser.Poison
        
        If you have `Poison` as a dependancy, then this adapter will be 
        selected by default.

        Returns an `MachineGun` structs as responses. or `MachineGun.Error`
        """
        ...
        defp format_request(request) do   
            %{
                options: %{:recv_timeout => 5000, :pool_group => :default},
